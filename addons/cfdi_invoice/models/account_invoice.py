...
_log = logging.getLogger("========= TIMBRADO =========")

class AccountInvoice(models.Model):
    _inherit = 'account.invoice'

    date_invoice = fields.Date('Fecha factura', readonly=True, states={'draft': [('readonly', False)]},default=lambda self: fields.Date.today(), copy=False)
    ...
    tipo_comprobante = fields.Selection([('I', 'Ingreso'), ('E', 'Egreso')], string='Tipo de comprobante',compute='_compute_tipo_comprobante', store=False)
    forma_pago = fields.Many2one('sat.forma.pago', string='Forma de pago', readonly=True, states={'draft': [('readonly', False)]},default=lambda self: self.get_default_id('cfdi_invoice.sat_forma_pago_03'))
    metodo_pago = fields.Many2one('sat.metodo.pago', string='Método de pago', readonly=True,states={'draft': [('readonly', False)]},default=lambda self: self.get_default_id('cfdi_invoice.sat_metodo_pago_PPD'))
    uso_cfdi = fields.Many2one('sat.uso.cfdi', string='Uso CFDI (cliente)', readonly=True,states={'draft': [('readonly', False)]},default=lambda self: self.get_default_id('cfdi_invoice.sat_uso_cfdi_P01'))
    tipo_relacion = fields.Many2one('sat.tipo.relacion', string='Tipo relación', copy=False)
    cfdis_relacionados = fields.Many2many('account.invoice', relation='account_invoice_cfdi_rel', column1='invoice_id',column2='invoice_id_rel', string='CFDIs Relacionados', copy=False)
    ...
    status_sat = fields.Char('Status SAT', readonly=True, copy=False, default='Sin validar')
    currency_exchange = fields.Float(string="Currency Exchange", compute="_compute_currency_exchange", store=False, digits=(18, 6))
    ...
    invoice_partner = fields.Char(string='Razón Social', default="_compute_get_name", readonly=False, copy=False)
    ...
    def gen_xml(self):
        ...
        for l in self.invoice_line_ids:
            importe = self.currency_id.round(l.quantity * l.price_unit)
            ...
            sum_descuentos = sum_descuentos + descuento
            concepto.set('Descuento', '%.6f' % self.currency_id.round(descuento))
            concepto.set('ValorUnitario', '%.6f' % l.price_unit)
            concepto.set('Importe', '%.6f' % importe)
        ...
        if len(traslados):
            impuestos.append(traslados)
        total = self.amount_untaxed + sum_traslados - sum_retenciones
        comprobante.set('Descuento', '%.2f' % sum_descuentos)
        comprobante.set('SubTotal', '%.2f' % (self.amount_untaxed + sum_descuentos))
    ...
    def timbrar(self):
        for rec in self:
            ...
            if result['validate']:
                ...
                qr_value = '%s?id=%s&re=%s&rr=%s&tt=0&fe=%s' % (url, UUID, rec.company_id.vat, rec.partner_id.vat, sello_digital[-8:])
    ...
    def cancel_cfdi(self):
        if self.cfdi and len(self.uuid.split('-')) == 5:
            ...
            result = pac.cancelar_cfdi(self, self.uuid)
            if not result['validate']:
                self.message_post(
                    body="Error de cancelación CFDI.\nCódigo: %s\n%s" % (result['code'], result['description']))
                return False
            pac.cancelaciones = pac.cancelaciones + 1
            self.message_post(
                body="Se cancela la factura con CFDI.\nCódigo: %s\n%s" % (result['code'], result['description']))
            return True
        else:
    ...
    def ConsultaCFDIService(self, invoice_ids):
        invoices = self.env["account.invoice"].browse(invoice_ids)
        for invoice in invoices:
            if invoice.cfdi:
                request = '?re=' + str(invoice.company_id.vat) + '&rr=' + str(
                    invoice.partner_id.vat) + '&tt=' + str(invoice.amount_total) + '&id=' + str(invoice.uuid)
                ...
                invoice.write({"status_sat": estado})

    def consulta_cfdi_service_cron(self):
        invoices = self.env["account.invoice"].search(['&', ('cfdi', '=', 'true'), ('status_sat', '=', 'Sin validar')])
        for invoice in invoices:
            if invoice.cfdi:
                request = '?re=' + str(invoice.company_id.vat) + '&rr=' + str(
                    invoice.partner_id.vat) + '&tt=' + str(invoice.amount_total) + '&id=' + str(invoice.uuid)
    ...
    def download_acuse(self):
        if self.cfdi and len(self.uuid.split('-')) == 5:
            ...
            if not result['validate']:
                self.message_post(
                    body="Error de acuse de cancelación CFDI.\nCódigo: %s\n%s" % (result['code'], result['description']))
                return False
            xml = result['xml']
            xml_obj = ET2.fromstring(base64.b64decode(xml))
            fecha_cancelado = ""
