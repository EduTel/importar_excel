...
class AccountPaymentInvoiceCFDI(models.Model):
    ...
    name = fields.Many2one('account.invoice', string='Factura',
    ...
    metodo_pago = fields.Many2one(
        'sat.metodo.pago', string='Método de pago', related='name.metodo_pago')
    currency_id = fields.Many2one(
        'res.currency', string='Moneda', related='name.currency_id')
    ...
    saldo = fields.Monetary(
        'Saldo Pendiente', compute='_get_saldo', store=False)
...
class AccountPayment(models.Model):
    ...
    fecha_emision = fields.Datetime(
        string="Fecha de Emision", default=lambda self: fields.Datetime.now())
    payment_date = fields.Date(
        string="Fecha de Emision", default=lambda self: fields.Date.today())
    cfdi = fields.Boolean('Timbrado', default=False, readonly=True, copy=False)
    fecha_pago = fields.Datetime(
        'Fecha de pago', required=True, default=lambda self: fields.Datetime.now(), copy=False)
    amount_total_text = fields.Char(
        'Total con letra', compute='_get_amount_total_text', store=False)
    forma_pago = fields.Many2one('sat.forma.pago', string='Forma de pago', default=lambda self: self.get_default_id(
        'cfdi_invoice.sat_forma_pago_03'), copy=False)
    uso_cfdi = fields.Many2one('sat.uso.cfdi', string='Uso CFDI (cliente)', default=lambda self: self.get_default_id(
        'cfdi_invoice.sat_uso_cfdi_P01'), readonly=True, copy=False)
    fecha_timbrado = fields.Char(
        'Fecha de Timbrado', readonly=True, copy=False)
    canceled_payment = fields.Many2one('account.payment', string='Pago reemplazo',
                                       help='Si este complemento de pago se relaciona con uno previamente cancelado, por favor seleccionelo aquí.', copy=False)
    canceled_payment_ids = fields.One2many(
        'account.payment', 'canceled_payment', string='Pagos cancelados relacionados', copy=False)
    payment_invoices_ids = fields.One2many(
        'account.payment.invoice.cfdi', 'payment_id', string='Facturas asociadas', copy=False)
    payment_invoices_total = fields.Monetary(
        'Suma de facturas pagadas', readonly=True, copy=False)
    amount_difference = fields.Monetary(
        'Diferencia de monto', readonly=True, copy=False)
    usuario_timbrado = fields.Many2one(
        'res.users', string='Timbrado por', readonly=True, copy=False)
    uuid = fields.Char('Factura UUID', readonly=True, copy=False)
    certificado_emisor = fields.Char(
        'Certificador emisor', readonly=True, copy=False)
    ...
    lugar_expedicion = fields.Char(
        'Lugar de expedición', readonly=True, copy=False)
    sello_digital = fields.Char('Sello digital', readonly=True, copy=False)
    sello_digital_sat = fields.Char(
        'Sello digital SAT', readonly=True, copy=False)
    ...
    move_ids = fields.Many2many(
        "account.move", compute="_compute_moves", store=False)
    cuenta_emisor = fields.Many2one(
        'res.partner.bank', string='Cuenta del emisor')
    banco_emisor = fields.Char(
        'Banco del emisor', related='cuenta_emisor.bank_name', readonly=True)
    rfc_banco_emisor = fields.Char(
        'RFC banco emisor', related='cuenta_emisor.bank_bic', readonly=True)
    ...
    status_sat = fields.Char('Estatus SAT', readonly=True, copy=False, default='Sin validar')
    ...
    def get_total(self, payment, invoice):
        ...
        if invoice.type in ('out_invoice', 'in_refund'):
            ...
            amount_currency = sum(
                [p.amount_currency for p in payment.matched_debit_ids if p.debit_move_id in invoice.move_id.line_ids])
            if payment.matched_debit_ids:
                payment_currency_id = all([p.currency_id == payment.matched_debit_ids[0].currency_id for p in payment.matched_debit_ids]
            ...
            amount_currency = sum(
                [p.amount_currency for p in payment.matched_credit_ids if p.credit_move_id in invoice.move_id.line_ids])
        ...
        if payment_currency_id and payment_currency_id == invoice.currency_id:
            ...
            amount_to_show = payment.company_id.currency_id.with_context(
                date=payment.date).compute(amount, invoice.currency_id)
    ...
    def compute_payment_invoices(self):
        ...
        for invoice in self.invoice_ids:
            ...
            total_pagos = total_pagos + invoice.currency_id.with_context(
                date=self.payment_date).compute(pago_actual, self.currency_id)
    ...
    def onchange_fecha_emision(self):
        if self.env.user.tz:
            ...
            date = fields.Datetime.from_string(self.fecha_emision).replace(
    ...
    def gen_xml(self):
        ...
        comprobante.set('NoCertificado',
                        self.env.user.branch_office.pac_config.no_cer)
        comprobante.set(
            'Certificado', self.env.user.branch_office.pac_config.cer.strip())
        comprobante.set('Moneda', 'XXX')
        comprobante.set('SubTotal', '0')
        comprobante.set('Total', '0')
        comprobante.set('TipoDeComprobante', 'P')
        comprobante.set('LugarExpedicion',
                        self.env.user.branch_office.partner.zip or '')
        if len(self.canceled_payment_ids) > 0:
            cfdi_relacionados = ET.Element('cfdi:CfdiRelacionados')
            cfdi_relacionados.set('TipoRelacion', self.env.ref(
                'cfdi_invoice.sat_tipo_relacion_04').code)
        ...
        if self.currency_id.name != "MXN":
            pago.set('TipoCambioP', "%.4f" % (
                1 / self.currency_id.with_context(date=self.fecha_pago).rate))
        ...
        for rec in self.payment_invoices_ids:
            ...
            doc.set('ImpPagado', "%.2f" % rec.pagado)
            doc.set('ImpSaldoInsoluto', "%.2f" % rec.saldo)
            pago.append(doc)
        pagos.append(pago)
        complemento.append(pagos)
        comprobante.append(complemento)
        xslt_file = os.path.dirname(os.path.realpath(__file__)).replace(
            'models', 'data_sat/cadenaoriginal_3_3.xslt')
        ...
        key = crypto.load_privatekey(
            crypto.FILETYPE_PEM, self.env.user.branch_office.pac_config.key_pem)
    ...
    def timbrar(self):
        for rec in self:
            ...
            if not pac or len(pac) == 0:
                raise Warning(
                    "Configure los datos del PAC correctamente para poder timbrar.")
            xml = rec.gen_xml()
            ...
            if result['validate']:
                ...
                xml_name = "%s.xml" % UUID
                TFD = xml_obj.find('cfdi:Complemento', NSMAP).find(
                ...
                qr_value = '%s?id=%s&re=%s&rr=%s&tt=0&fe=%s' % (
                    url, UUID, self.env.user.company_id.vat, rec.partner_id.vat, sello_digital[-8:])
                qrcode = rec.get_cbb(qr_value)
                xslt_file = os.path.dirname(os.path.realpath(__file__)).replace(
                    'models', 'data_sat/cadenaoriginal_TFD_1_1.xslt')
                xslt = ET2.parse(xslt_file)
                transform = ET2.XSLT(xslt)
                ...
                raise Warning("%s\n%s" %
                              (error, unicode(xml, errors='ignore')))
    ...
    def ConsultaCFDIServicePayment(self, invoice_ids):
        payments = self.env["account.payment"].browse(invoice_ids)
        for payment in payments:
            if payment.cfdi:
                request = '?re=' + str(payment.company_id.vat) + '&rr=' + str(
                    payment.partner_id.vat) + '&tt=' + str('0') + '&id=' + str(payment.uuid)
                ...
                if(result.Estado == 'No Encontrado'):
                    estado = 'No Encontrado/Verificar en página del SAT'
                payment.write({"status_sat": estado})

    def consulta_cfdi_service_payment_cron(self):
        payments = self.env["account.payment"].search(['&', ('cfdi', '=', 'true'), ('status_sat', '=', 'Sin validar')])
        for payment in payments:
            if payment.cfdi:
                request = '?re=' + str(payment.company_id.vat) + '&rr=' + str(
                    payment.partner_id.vat) + '&tt=' + str('0') + '&id=' + str(payment.uuid)
