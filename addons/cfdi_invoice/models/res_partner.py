...
class ResPartner(models.Model):
    ...
    forma_pago = fields.Many2one('sat.forma.pago', string='Forma de pago',
                                 default=lambda self: self.get_default_id('cfdi_invoice.sat_forma_pago_03'))
    metodo_pago = fields.Many2one('sat.metodo.pago', string='Método de pago',
                                  default=lambda self: self.get_default_id('cfdi_invoice.sat_metodo_pago_PPD'))
    uso_cfdi = fields.Many2one('sat.uso.cfdi', string='Uso CFDI (cliente)',
    ...
    @api.constrains('vat')
    def valida_RFC(self):
        logging.getLogger("_____OK_____").info(
            "%s, %s, %s, %s", self.name, self.type, self.company_type, self.parent_id)
        if self.parent_id == False:
            if self.vat == False:
                raise Warning(
                    "Error de Validacion : El cliente %s no tiene ningun RFC asignado, favor de asignarlo primero" % (self.name))
            else:
